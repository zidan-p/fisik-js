...
export abstract class LineSegmentOld {
  ...
  static closestPointPositionToLineSegment(position: Vector, wallLike: LineSegmentOld){
  ...
  static closestPointBetweenLineSegemnt(ls1: LineSegmentOld, ls2: LineSegmentOld){
    let shortestDistance = LineSegmentOld.closestPointPositionToLineSegment(ls1.start, ls2)
      .subtr(ls1.start).mag();
    let closestPoint = [ls1.start, LineSegmentOld.closestPointPositionToLineSegment(ls1.start, ls2)];

    if(LineSegmentOld.closestPointPositionToLineSegment(ls1.end, ls2).subtr(ls1.end).mag() < shortestDistance){
      shortestDistance = LineSegmentOld.closestPointPositionToLineSegment(ls1.end, ls2).subtr(ls1.end).mag();
      closestPoint = [ls1.end, LineSegmentOld.closestPointPositionToLineSegment(ls1.end, ls2)];
    }  

    if(LineSegmentOld.closestPointPositionToLineSegment(ls2.start, ls1).subtr(ls2.start).mag() < shortestDistance){
      shortestDistance = LineSegmentOld.closestPointPositionToLineSegment(ls2.start, ls1).subtr(ls2.start).mag();
      closestPoint = [LineSegmentOld.closestPointPositionToLineSegment(ls2.start, ls1), ls2.start];
    }

    if(LineSegmentOld.closestPointPositionToLineSegment(ls2.end, ls1).subtr(ls2.end).mag() < shortestDistance){
      shortestDistance = LineSegmentOld.closestPointPositionToLineSegment(ls2.end, ls1).subtr(ls2.end).mag();
      closestPoint = [LineSegmentOld.closestPointPositionToLineSegment(ls2.end, ls1), ls2.end];
    }

    return closestPoint;
  }

  static findAxes(ls1: LineSegmentOld, ls2: LineSegmentOld){
    const axes: Vector[] = [];
  ...
  static projectionShapeOntoAxis(axis: Vector, object: LineSegmentOld): Projection{
  ...
  static sat(o1: LineSegmentOld, o2: LineSegmentOld){
    let minOverlap: null | number = null;
    let smallestAxis: Vector;
    let vertexObject : LineSegmentOld;

    const axes = LineSegmentOld.findAxes(o1, o2);
    ...
    for (let index = 0; index < axes.length; index++) {
      projection1 = LineSegmentOld.projectionShapeOntoAxis(axes[index], o1);
      projection2 = LineSegmentOld.projectionShapeOntoAxis(axes[index], o2);
    ...
    // beware !!!
    const contactVertex = LineSegmentOld.projectionShapeOntoAxis(smallestAxis!, vertexObject!).collisionVertex;
